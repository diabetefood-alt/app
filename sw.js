...
const CACHE_NAME = 'diabetefood-v83';
const ASSETS = [
  '/app/',
  '/app/index.html',
  '/app/manifest.json',
...
self.addEventListener('install', event => {
  event.waitUntil(
    caches.open(CACHE_NAME).then(cache => {
      // Fetch avec cache-busting pour eviter le HTTP cache du navigateur
...
self.addEventListener('fetch', event => {
  ...
  if (isHTML) {
    // Pour les requetes HTML : network-first avec injection de scripts
    event.respondWith(
    ...
    // Pour les autres requetes : network-first avec cache-busting
    event.respondWith(
      fetch(event.request, {cache: 'no-store'})
        .then(response => {
          if (response.ok) {
